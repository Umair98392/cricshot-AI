    ...
    predictButton.addEventListener("click", async function () {
        ...
        if (fileInput) {
            ...
            try {
                ...
                const response = await fetch("https://noble-gently-barnacle.ngrok-free.app/predict", { method: "POST",body: formData});
                ...
                if (response.ok) {
                    ...
                    for (let i = 0; i < predictedImages.length; i++) {
                        predictedImages[i].src = `data:image/png;base64,${data[`result_image_${i + 1}`]}`;
                    }
